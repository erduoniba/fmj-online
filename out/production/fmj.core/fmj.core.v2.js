...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (root, factory) {
  ...
  function Combat(parent) {
    ...
    this.mLastPlayerActions_0 = LinkedHashMap_init();
  ...
  Combat.prototype.prepareForNewCombat_0 = function () {
    ...
    this.mLastPlayerActions_0.clear();
    this.mIsAutoAttack_0 = false;
    this.mCombatState_0 = Combat$CombatState$SelectAction_getInstance();
    this.mCurSelActionPlayerIndex_0 = 0;
  ...
  Combat.prototype.onKeyDown_za3lpa$ = function (key) {
    ...
    if (this.mCombatState_0 === Combat$CombatState$SelectAction_getInstance()) {
      if (!this.mIsAutoAttack_0) {
        if (key === Global_getInstance().KEY_REPEAT) {
          this.handleRepeatAction_0();
        } else {
  ...
  Combat.prototype.onActionSelected_wwcj9m$ = function (action) {
    var $receiver = this.mLastPlayerActions_0;
    var key = this.mCurSelActionPlayerIndex_0;
    $receiver.put_xwzc9p$(key, action);
  ...
  Combat.prototype.handleRepeatAction_0 = function () {
    var tmp$;
    var currentPlayer = this.mPlayerList_0.get_za3lpa$(this.mCurSelActionPlayerIndex_0);
    if (!currentPlayer.isAlive || currentPlayer.isSleeping) {
      return;
    }
    var lastAction = this.mLastPlayerActions_0.get_11rb$(this.mCurSelActionPlayerIndex_0);
    if (lastAction != null) {
      if (Kotlin.isType(lastAction, ActionPhysicalAttackOne)) {
        var target = this.firstAliveMonster;
        if (target != null) {
          tmp$ = new ActionPhysicalAttackOne(currentPlayer, target);
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionPhysicalAttackAll))
        if (currentPlayer.hasAtbuff_za3lpa$(FightingCharacter$Companion_getInstance().BUFF_MASK_ALL)) {
          tmp$ = new ActionPhysicalAttackAll(currentPlayer, this.mMonsterList_0);
        } else
          tmp$ = null;
      else if (Kotlin.isType(lastAction, ActionMagicAttackOne)) {
        var magic = lastAction.magic_8be2vx$;
        if (currentPlayer.mp >= magic.costMp && !currentPlayer.isSealed) {
          var target_0 = this.firstAliveMonster;
          if (target_0 != null) {
            tmp$ = new ActionMagicAttackOne(currentPlayer, target_0, magic);
          } else
            tmp$ = null;
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionMagicAttackAll)) {
        var magic_0 = lastAction.magic_8be2vx$;
        if (currentPlayer.mp >= magic_0.costMp && !currentPlayer.isSealed) {
          tmp$ = new ActionMagicAttackAll(currentPlayer, this.mMonsterList_0, magic_0);
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionMagicHelpOne)) {
        var magic_1 = lastAction.magic_8be2vx$;
        if (currentPlayer.mp >= magic_1.costMp && !currentPlayer.isSealed) {
          var $receiver = this.mPlayerList_0;
          var destination = ArrayList_init();
          var tmp$_0;
          tmp$_0 = $receiver.iterator();
          ...
          minByOrNull$break: do {
           ...
           while (false);
          var target_1 = minByOrNull$result;
          if (target_1 != null) {
            tmp$ = new ActionMagicHelpOne(currentPlayer, target_1, magic_1);
          } else
            tmp$ = null;
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionMagicHelpAll)) {
        var magic_2 = lastAction.magic_8be2vx$;
        if (currentPlayer.mp >= magic_2.costMp && !currentPlayer.isSealed) {
          tmp$ = new ActionMagicHelpAll(currentPlayer, this.mPlayerList_0, magic_2);
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionUseItemOne)) {
        var item = lastAction.goods_8be2vx$;
        if (Player$Companion_getInstance().sGoodsList.getGoodsNum_vux9f0$(item.type, item.index) > 0) {
          var $receiver_0 = this.mPlayerList_0;
          var destination_0 = ArrayList_init();
          var tmp$_1;
          ...
          var minByOrNull$result_0;
          minByOrNull$break: do {
            var iterator_0 = destination_0.iterator();
            if (!iterator_0.hasNext()) {
              minByOrNull$result_0 = null;
              break minByOrNull$break;
            }
            var minElem_0 = iterator_0.next();
            if (!iterator_0.hasNext()) {
              minByOrNull$result_0 = minElem_0;
              break minByOrNull$break;
            }
            var minValue_0 = minElem_0.hp;
            do {
           ...
           while (false);
          var target_2 = minByOrNull$result_0;
          if (target_2 != null) {
            tmp$ = new ActionUseItemOne(currentPlayer, target_2, item);
          } else
            tmp$ = null;
        } else
          tmp$ = null;
      } else if (Kotlin.isType(lastAction, ActionUseItemAll)) {
        var item_0 = lastAction.goods_8be2vx$;
        if (Player$Companion_getInstance().sGoodsList.getGoodsNum_vux9f0$(item_0.type, item_0.index) > 0) {
          tmp$ = new ActionUseItemAll(currentPlayer, this.mPlayerList_0, item_0);
        } else
          tmp$ = null;
      } else
        tmp$ = null;
      var repeatedAction = tmp$;
      if (repeatedAction != null) {
        this.onActionSelected_wwcj9m$(repeatedAction);
        return;
      }
    }
    var target_3 = this.firstAliveMonster;
    if (target_3 != null) {
      this.onActionSelected_wwcj9m$(new ActionPhysicalAttackOne(currentPlayer, target_3));
    }
  };
  ...
  ActionMagicAttackAll.prototype.preproccess = function () {
    ...
    this.mAni_0 = this.magic_8be2vx$.magicAni;
    ensureNotNull(this.mAni_0).start();
    ensureNotNull(this.mAni_0).setIteratorNum_za3lpa$(2);
    this.magic_8be2vx$.use_h32lzv$(attacker, this.mTargets);
  ...
  ActionMagicAttackOne.prototype.preproccess = function () {
    ...
    if (Kotlin.isType(this.magic_8be2vx$, MagicAttack)) {
      this.magic_8be2vx$.use_qwqr58$(attacker, target);
    }
    ...
    this.mRaiseAnimations.add_11rb$(attacker.diffToAnimation_6taknv$());
    if (Kotlin.isType(this.magic_8be2vx$, MagicSpecial)) {
  ...
  function DatLib$Companion$instance$lambda() {
    var choiceLibName = window.choiceLibName || 'FMJ';
    return new DatLib(File$Companion_getInstance().contentsOf_61zpoe$(choiceLibName + '.LIB'));
  }
